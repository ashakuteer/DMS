...
// Friendly display labels for home names coming from API
...
interface QueuedDoc { key: string; type: string; file: File }
...
export default function NewStaffPage() {
  ...
  const setB = (field: string, value: string) => setBank((p) => ({ ...p, [field]: value }));
  ...
  const uploadFile = useCallback(
    async (endpoint: string, fd: FormData) => {
  ...
  const validate = () => {
    ...
    if (!form.homeId) errs.homeId = "Home is required";
    setErrors(errs);
    ...
    <div className="p-6 max-w-3xl space-y-6">
        ...
        <CardContent>
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2">
              <Button variant="outline" size="sm" onClick={() => photoInputRef.current?.click()} data-testid="button-select-photo">
                <Upload className="mr-2 h-4 w-4" />
                {photoFile ? "Change Photo" : "Select Photo"}
              </Button>
              {photoFile && (
                <div className="flex items-center gap-2">
                  <p className="text-xs text-muted-foreground truncate max-w-[160px]">{photoFile.name}</p>
                  <button
          ...
          <input ref={photoInputRef} type="file" accept="image/*" className="hidden" onChange={handlePhotoSelect} data-testid="input-photo-file" />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          {/* Designation */}
          <div className="space-y-1.5">
            <Label htmlFor="select-designation">Designation *</Label>
            <Select
              value={form.designation}
              onValueChange={(v) => setF("designation", v)}
            >
              <SelectTrigger
          ...
          {/* Home */}
          <div className="space-y-1.5">
            <Label htmlFor="select-home">Home *</Label>
            <Select
              value={form.homeId}
              onValueChange={(v) => setF("homeId", v)}
            >
              <SelectTrigger
                id="select-home"
                className={errors.homeId ? "border-destructive" : ""}
            ...
            {errors.homeId && <p className="text-xs text-destructive">{errors.homeId}</p>}
          </div>

          {/* Blood Group */}
          <div className="space-y-1.5">
            <Label htmlFor="select-blood-group">Blood Group</Label>
            <Select value={form.bloodGroup} onValueChange={(v) => setF("bloodGroup", v)}>
              <SelectTrigger id="select-blood-group" data-testid="select-blood-group">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <input ref={docInputRef} type="file" accept="image/*,.pdf" className="hidden" onChange={handleDocSelect} data-testid="input-doc-file" />
          {queuedDocs.length > 0 && (
            <div className="space-y-2">
              {queuedDocs.map((doc) => (
                <div key={doc.key} className="flex items-center justify-between gap-3 rounded-lg border px-3 py-2 bg-muted/30" data-testid={`queued-doc-${doc.key}`}>
                  <div className="flex items-center gap-2 min-w-0">
                    <FileText className="h-4 w-4 text-muted-foreground shrink-0" />
                    <Badge variant="outline" className="text-xs shrink-0">{DOC_TYPES[doc.type]}</Badge>
                    <span className="text-sm text-muted-foreground truncate">{doc.file.name}</span>
                  </div>
                  <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0 text-muted-foreground hover:text-destructive" onClick={() => removeDoc(doc.key)} data-testid={`button-remove-doc-${doc.key}`}>
