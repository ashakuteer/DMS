...
var HomeSummaryService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.HomeSummaryService = void 0;
...
let HomeSummaryService = HomeSummaryService_1 = class HomeSummaryService {
    constructor(prisma, orgProfileService) {
        this.prisma = prisma;
        this.orgProfileService = orgProfileService;
        this.logger = new common_1.Logger(HomeSummaryService_1.name);
    }
    homeLabel(type) {
        const map = {
    ...
    async getSummary(month, year) {
        ...
        const homeTypes = Object.values(client_1.HomeType);
        const homes = [];
        for (const homeType of homeTypes) {
            ...
            const activeBeneficiaries = allBeneficiaries.filter(b => b.status === client_1.BeneficiaryStatus.ACTIVE);
            const inactiveBeneficiaries = allBeneficiaries.filter(b => b.status === client_1.BeneficiaryStatus.INACTIVE);
            ...
            const sickBeneficiaryIds = new Set();
            const hospitalizedBeneficiaryIds = new Set();
            for (const event of healthEvents) {
                if (event.severity === client_1.HealthEventSeverity.CRITICAL) {
                    hospitalizedBeneficiaryIds.add(event.beneficiaryId);
                }
                else if (event.severity === client_1.HealthEventSeverity.HIGH || event.severity === client_1.HealthEventSeverity.MEDIUM) {
            ...
            const exitedBeneficiaries = await this.prisma.beneficiary.count({
                where: {
                    homeType,
                    isDeleted: false,
                    status: client_1.BeneficiaryStatus.INACTIVE,
    ...
    pdfTable(doc, headers, rows, highlightLast = false) {
...
exports.HomeSummaryService = HomeSummaryService;
exports.HomeSummaryService = HomeSummaryService = HomeSummaryService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        organization_profile_service_1.OrganizationProfileService])
], HomeSummaryService);
//# sourceMappingURL=home-summary.service.js.map