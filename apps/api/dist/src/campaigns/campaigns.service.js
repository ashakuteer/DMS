...
let CampaignsService = CampaignsService_1 = class CampaignsService {
    ...
    async findOne(id) {
        ...
        const totalRaised = campaign.donations.reduce((sum, d) => sum + Number(d.donationAmount), 0);
    ...
    async create(dto, user) {
        return this.prisma.campaign.create({
            data: {
                ...
                status: dto.status || client_1.CampaignStatus.DRAFT,
    ...
    async update(id, dto) {
        ...
        if (dto.goalAmount !== undefined)
            data.goalAmount = dto.goalAmount;
        ...
        if (dto.homeTypes !== undefined)
            data.homeTypes = dto.homeTypes;
    ...
    async addBeneficiaries(campaignId, beneficiaryIds, notes) {
    ...
    async createUpdate(campaignId, dto, user) {
        const campaign = await this.prisma.campaign.findUnique({ where: { id: campaignId } });
        if (!campaign || campaign.isDeleted) {
            throw new common_1.NotFoundException('Campaign not found');
        }
    ...
    async deleteUpdate(campaignId, updateId) {
    ...
    async getDonors(campaignId) {
        ...
        for (const d of donations) {
            ...
            if (existing) {
                ...
                if (date > existing.lastDonation)
                    existing.lastDonation = date;
                if (date < existing.firstDonation)
                    existing.firstDonation = date;
            }
            else {
    ...
    async getAnalytics(campaignId) {
        ...
        for (const d of donations) {
            ...
            if (!byHome[home])
                byHome[home] = { home, amount: 0, count: 0 };
    ...
    async generateWhatsAppAppeal(campaign) {
    ...
    async sendEmailAppeal(id, donorIds, user) {
        const campaign = await this.findOne(id);
        if (!donorIds || donorIds.length === 0) {
            throw new common_1.BadRequestException('Please select at least one donor');
        }
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>Warm regards,<br/>${orgName}</p>
    ...
    async broadcastUpdate(campaignId, updateId, donorIds, user) {
        const campaign = await this.prisma.campaign.findUnique({ where: { id: campaignId } });
        if (!campaign || campaign.isDeleted) {
            throw new common_1.NotFoundException('Campaign not found');
        }
        ...
        <div style="display: flex; justify-content: space-between; font-size: 13px; color: #777; margin-top: 4px;">
          ...
          <span>Goal: Rs. ${goalAmount.toLocaleString('en-IN')}</span>
            ...
            const photoHtml = update.photoUrls && update.photoUrls.length > 0
                ? `<div style="margin: 16px 0;">${update.photoUrls.map(url => `<img src="${url}" alt="Campaign update" style="max-width: 100%; border-radius: 8px; margin-bottom: 8px;" />`).join('')}</div>`
                : '';
            const body = `
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>Warm regards,<br/>${orgName}</p>
            ...
            await this.prisma.campaignUpdateDispatch.create({
                data: {
                    ...
                    donorId: donor.id,
                    channel: client_1.SponsorDispatchChannel.EMAIL,
                    status: client_1.SponsorDispatchStatus.SENT,
                    sentAt: new Date(),
                },
            });
            await this.prisma.communicationLog.create({
    ...
    async logWhatsAppBroadcast(campaignId, updateId, donorId, user) {
        const campaign = await this.prisma.campaign.findUnique({ where: { id: campaignId } });
        if (!campaign || campaign.isDeleted) {
            throw new common_1.NotFoundException('Campaign not found');
        }
        ...
        await this.prisma.campaignUpdateDispatch.create({
            data: {
                campaignUpdateId: update.id,
                donorId: donor.id,
                channel: client_1.SponsorDispatchChannel.WHATSAPP,
                status: client_1.SponsorDispatchStatus.COPIED,
                sentAt: new Date(),
            },
        });
        await this.prisma.communicationLog.create({
    ...
    async getUpdateWhatsAppText(campaignId, updateId) {
        const campaign = await this.prisma.campaign.findUnique({ where: { id: campaignId } });
        if (!campaign || campaign.isDeleted) {
            throw new common_1.NotFoundException('Campaign not found');
        }
    ...
    async getUpdateDispatches(campaignId, updateId) {
        const campaign = await this.prisma.campaign.findUnique({ where: { id: campaignId } });
        if (!campaign || campaign.isDeleted) {
            throw new common_1.NotFoundException('Campaign not found');
        }
    ...
    async getCampaignTotalRaised(campaignId) {
        const donations = await this.prisma.donation.findMany({
            where: { campaignId, isDeleted: false },
