...
const donation_templates_1 = require("./templates/donation.templates");
let EmailService = EmailService_1 = class EmailService {
    constructor(orgProfileService) {
        ...
        this.emailRelayUrl = null;
        this.emailRelaySecret = null;
    ...
    async wrapWithBranding(bodyHtml) {
        const org = await this.orgProfileService.getProfile();
    ...
    async sendReminderEmail(toEmail, donorName, reminderType, relatedPersonName) {
        const org = await this.orgProfileService.getProfile();
        const phoneDisplay = org.phone2 ? `${org.phone1} / ${org.phone2}` : org.phone1;
        let subject;
        let bodyText;
...
(This is an automated email. Please do not reply.)`;
    ...
    async sendDonationReceipt(toEmail, donorName, receiptNumber, pdfBuffer, options) {
        const org = await this.orgProfileService.getProfile();
        const logoFilePath = (0, pdf_branding_1.resolveLogoPath)(org.logoUrl || null);
        const emailType = options?.emailType || 'GENERAL';
        const template = (0, donation_templates_1.getDonationEmailTemplate)(emailType, {
            donorName,
            receiptNumber,
    ...
    async sendEmailWithInline(options) {
        const testEmail = process.env.TEST_EMAIL;
        if (testEmail && options.to !== testEmail) {
            this.logger.log(`[TEST_EMAIL] Redirecting email from ${options.to} → ${testEmail}`);
            options = { ...options, to: testEmail };
