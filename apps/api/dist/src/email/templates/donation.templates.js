...
exports.getGeneralThankYouTemplate = getGeneralThankYouTemplate;
exports.getTaxReceiptTemplate = getTaxReceiptTemplate;
exports.getKindDonationTemplate = getKindDonationTemplate;
exports.getDonationEmailTemplate = getDonationEmailTemplate;
const NAVY = '#1F3B64';
const WHITE = '#FFFFFF';
...
function baseLayout(params, bodyContent) {
    const { org } = params;
...
function getGeneralThankYouTemplate(params) {
    ...
    if (donationAmount)
        rows.push({ label: 'Donation Amount', value: formatAmount(donationAmount, currency) });
    if (donationDate)
        rows.push({ label: 'Date', value: formatDate(donationDate) });
    if (donationMode)
        rows.push({ label: 'Payment Mode', value: formatMode(donationMode) });
    const body = `
...
function getTaxReceiptTemplate(params) {
    ...
    if (donationMode)
        rows.push({ label: 'Payment Mode', value: formatMode(donationMode) });
    if (donorPAN)
        rows.push({ label: "Donor's PAN", value: donorPAN });
    const body = `
    <p style="margin:0 0 20px 0;font-size:16px;color:${CHARCOAL};line-height:1.5;">
      Dear <strong>${donorName}</strong>,
    </p>
...
function getKindDonationTemplate(params) {
...
function getDonationEmailTemplate(type, params) {
    switch (type) {
        case 'TAX':
...
//# sourceMappingURL=donation.templates.js.map