...
var DashboardActionsService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DashboardActionsService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../prisma/prisma.service");
const communication_log_service_1 = require("../communication-log/communication-log.service");
const client_1 = require("@prisma/client");
let DashboardActionsService = DashboardActionsService_1 = class DashboardActionsService {
    constructor(prisma, communicationLogService) {
        this.prisma = prisma;
        this.communicationLogService = communicationLogService;
        this.logger = new common_1.Logger(DashboardActionsService_1.name);
        this.cache = new Map();
    ...
    async getStaffActions() {
        const cached = this.getCached("staff_actions");
        if (cached) {
    ...
    async getDailyActions() {
        ...
        const todaySpecialDays = [];
        const upcoming7Days = [];
        const upcoming15Days = [];
        ...
        const mapReminder = (r) => ({
            ...
            daysOverdue: new Date(r.dueDate) < todayStart
                ? Math.ceil((todayStart.getTime() - new Date(r.dueDate).getTime()) /
                    (1000 * 60 * 60 * 24))
                : 0,
            daysUntil: new Date(r.dueDate) >= todayStart
                ? Math.ceil((new Date(r.dueDate).getTime() - todayStart.getTime()) /
                    (1000 * 60 * 60 * 24))
                : 0,
            donor: r.donor,
        });
        ...
        const upcoming7Pledges = pledgesDue
            ...
            .map(mapPledge);
        const beneficiaryBirthdays = [];
        for (const beneficiary of beneficiariesWithBirthdays) {
            if (!beneficiary.dobMonth || !beneficiary.dobDay)
                continue;
            ...
            if (daysUntil >= 0 && daysUntil <= 7) {
                beneficiaryBirthdays.push({
                    ...
                    sponsors: beneficiary.sponsorships.map((s) => ({
                        donorId: s.donorId,
        ...
        const monthlySponsorshipsDue = [];
        for (const sponsorship of activeMonthlySponsors) {
            const startDay = sponsorship.dueDayOfMonth ??
                (sponsorship.startDate ? new Date(sponsorship.startDate).getDate() : null);
            if (startDay === null)
                continue;
            let dueDate;
            if (sponsorship.nextDueDate) {
                dueDate = new Date(sponsorship.nextDueDate);
            }
            else {
        ...
        return {
            todaySpecialDays: {
                birthdays: todaySpecialDays.filter((o) => o.type === "DOB_SELF" ||
                    o.type === "DOB_SPOUSE" ||
                    o.type === "DOB_CHILD"),
    ...
    async markActionDone(user, params) {
    ...
    async snoozeAction(user, params) {
...
exports.DashboardActionsService = DashboardActionsService;
exports.DashboardActionsService = DashboardActionsService = DashboardActionsService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService,
        communication_log_service_1.CommunicationLogService])
], DashboardActionsService);
//# sourceMappingURL=dashboard.actions.service.js.map