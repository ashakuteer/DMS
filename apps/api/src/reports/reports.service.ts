...
function calculateDonorHealth(lastDonationDate: Date | null): DonorHealth {
...
export class ReportsService {
  ...
  private getFYDates(fyType: 'current' | 'last'): { start: Date; end: Date } {
    const now = new Date();
  ...
  async getDonorSummary(
    ...
    const data = donors.map((donor) => {
      ...
      return {
        ...
        fyTotal: fyDonations.reduce((sum: number, d) => sum + (d.donationAmount?.toNumber() || 0), 0),
        fyCount: fyDonations.length,
        lifetimeTotal: allDonations.reduce((sum: number, d) => sum + (d.donationAmount?.toNumber() || 0), 0),
  ...
  async getDonorReport(
    ...
    // Sort in memory (for proper numeric/date sorting)
    data.sort((a, b) => {
  ...
  async exportDonorReportExcel(filter: DateFilter): Promise<Buffer> {
    ...
    // Add summary row
    sheet.addRow({});
    const totalLifetime = allDonors.data.reduce((sum: number, d: any) => sum + d.lifetimeTotal, 0);
    ...
    const summaryRow = sheet.addRow({
      ...
      fyTotal: totalFY,
      donationCount: allDonors.data.reduce((sum: number, d: any) => sum + d.donationCount, 0),
    });
  ...
  async exportDonorReportPdf(filter: DateFilter): Promise<Buffer> {
    ...
    return new Promise((resolve) => {
      ...
      const maxRows = 35;
      allDonors.data.slice(0, maxRows).forEach((d: any, index: number) => {
        if (y > 520) {
          doc.addPage();
          y = 40;
        }
      ...
      const totalLifetime = allDonors.data.reduce((sum: number, d: any) => sum + d.lifetimeTotal, 0);
      const totalFY = allDonors.data.reduce((sum: number, d: any) => sum + d.fyTotal, 0);
      const healthyCt = allDonors.data.filter((d: any) => d.healthStatus === 'HEALTHY').length;
      const atRiskCt = allDonors.data.filter((d: any) => d.healthStatus === 'AT_RISK').length;
      const dormantCt = allDonors.data.filter((d: any) => d.healthStatus === 'DORMANT').length;
  ...
  async exportReceiptsAuditExcel(filter: DateFilter, paymentMode?: string): Promise<Buffer> {
    ...
    sheet.addRow({});
    const totalAmount = allReceipts.data.reduce((sum: number, d: any) => sum + (d.amount?.toNumber?.() || d.amount || 0), 0);
    const summaryRow = sheet.addRow({
      receiptNumber: 'TOTAL',
