...
export class DonorsEngagementService {
  ...
  private async computeEngagementScoresChunk(
    ...
    for (const donorId of donorIds) {
      ...
      const count12Mo = donorDonations.filter(
        (d) => d.donationDate >= oneYearAgo,
      ).length;
      ...
      const ltv = donorDonations.reduce(
      ...
      const overdue = donorPledges.filter(
        (p) => p.expectedFulfillmentDate && p.expectedFulfillmentDate < now,
      );
