...
@Injectable()
export class DonorsExportService {
  constructor(
  ...
  async exportMasterDonorExcel(
    ...
    if (filters.home && filters.home !== "all") {
      filtered = filtered.filter(
        (d) =>
          d.donations.some(
            (don: any) => don.donationHomeType === filters.home,
          ) ||
          d.sponsorships.some(
            (s: any) => s.beneficiary?.homeType === filters.home,
          ),
      );
    }
    ...
    sheet.columns = [
      ...
      { header: "Permanent Address", key: "permanentStreetAddress", width: 30 },
      { header: "Permanent City", key: "permanentCity", width: 16 },
      { header: "Permanent State", key: "permanentState", width: 14 },
      { header: "Permanent Country", key: "permanentCountry", width: 12 },
      { header: "Permanent Pincode", key: "permanentPincode", width: 10 },
      { header: "Current Address", key: "currentStreetAddress", width: 30 },
      { header: "Current City", key: "currentCity", width: 16 },
      { header: "Current State", key: "currentState", width: 14 },
      { header: "Current Country", key: "currentCountry", width: 12 },
      { header: "Current Pincode", key: "currentPincode", width: 10 },
      { header: "Address (Legacy)", key: "address", width: 30 },
      { header: "City (Legacy)", key: "city", width: 16 },
      { header: "State (Legacy)", key: "state", width: 14 },
      { header: "Country (Legacy)", key: "country", width: 12 },
      { header: "Pincode (Legacy)", key: "pincode", width: 10 },
    ...
    for (const d of filtered) {
      ...
      const sponsorshipMonthly = d.sponsorships
        ...
        .reduce(
          (sum: number, s: any) => sum + (s.amount ? Number(s.amount) : 0),
          0,
        );
      ...
      sheet.addRow({
        ...
        permanentStreetAddress: d.permanentStreetAddress || d.address || "",
        permanentCity: d.permanentCity || d.city || "",
        permanentState: d.permanentState || d.state || "",
        permanentCountry: d.permanentCountry || d.country || "",
        permanentPincode: d.permanentPincode || d.pincode || "",
        currentStreetAddress: d.currentSameAsPermanent ? (d.permanentStreetAddress || d.address || "") : (d.currentStreetAddress || ""),
        currentCity: d.currentSameAsPermanent ? (d.permanentCity || d.city || "") : (d.currentCity || ""),
        currentState: d.currentSameAsPermanent ? (d.permanentState || d.state || "") : (d.currentState || ""),
        currentCountry: d.currentSameAsPermanent ? (d.permanentCountry || d.country || "") : (d.currentCountry || ""),
        currentPincode: d.currentSameAsPermanent ? (d.permanentPincode || d.pincode || "") : (d.currentPincode || ""),
        address: d.address || "",
        city: d.city || "",
        state: d.state || "",
        country: d.country || "",
