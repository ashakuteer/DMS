...
export class PledgesService {
  ...
  constructor(
    ...
    private remindersService: PledgesRemindersService,
  ) {}
  ...
  async sendPledgeReminderEmail(user: UserContext, id: string) {
    return this.remindersService.sendPledgeReminderEmail(user, id);
  }

  async buildWhatsAppReminderText(pledge: any): Promise<string> {
    return this.remindersService.buildWhatsAppReminderText(pledge);
  }

  async logWhatsAppReminder(user: UserContext, id: string) {
    return this.remindersService.logWhatsAppReminder(user, id);
  }

  private async generatePledgeReminders(pledgeId: string, donorId: string, dueDate: Date, pledgeType: PledgeType) {
    return this.remindersService.generatePledgeReminders(pledgeId, donorId, dueDate, pledgeType);
  }
}
